...
export default function ProfilePage() {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      if (initials.trim().slice(0, INITIALS_MAX)) {
        await updateLeaderboardInitials(user.id, initials.trim().slice(0, INITIALS_MAX));
      }
      setProfile((prev) => ({
        ...prev,
  ...
  const subtitle =
    title && company ? `${title} at ${company}` : title || company || 'Glossary Streak Player';

  return (
    <div
      className="h-full min-h-full bg-cover bg-center bg-no-repeat p-16"
      ...
      <div className="max-w-2xl bg-zinc-700/40 backdrop-blur px-10 py-8 rounded-2xl shadow-[0_10px_30px_rgba(0,0,0,0.08)]">
        <div className="flex justify-between items-center pb-10">
          <Link href="/" className="text-white hover:text-white/90 transition">
            <img src="/images/minecart.svg" alt="Home" className="h-8 w-8 opacity-60" />
          </Link>
          <div>
            <img src="/images/profile.svg" alt="Profile" className="w-[66%] mx-auto" />
          </div>
          <Link href="/play" className="text-white hover:text-white/90 transition">
            <img src="/images/enter.svg" alt="Play" className="h-8 w-8 opacity-60" />
          </Link>
        </div>
        ...
        <div className="flex flex-col">
          ...
          <div className="grid gap-4 mb-2">
            ...
            <div>
              ...
              <input
                ...
                value={initials}
                onChange={(e) => setInitials(e.target.value.toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, INITIALS_MAX))}
                placeholder="ABC"
          ...
          <p className="text-sm text-white/60 mb-6">
            Shows as: <span className="text-white/90">{displayName.trim() || user.name}</span>
            {subtitle !== 'Glossary Streak Player' && (
              <> · {subtitle}</>
            )}
          </p>
